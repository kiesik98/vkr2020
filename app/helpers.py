...
def process_form(test_question, request):
    if len(request.POST.getlist('answers', [])) > 0 or request.POST.get('text', None) not in ['', None]:
        ...
        if (
                type(request.POST.getlist('answers')[0]) == str
                and ',' in request.POST.getlist('answers')[0]
        ):
            answer2 = request.POST.getlist('answers')[0]
            is_answer = models.Answer.objects.filter(text=answer2)
        else:
            is_answer = models.Answer.objects.filter(id__in=request.POST.getlist('answers'))
